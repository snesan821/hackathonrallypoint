...
export default function ProfilePage() {
  ...
  const fetchSavedItems = async () => {
    try {
      ...
      const data = await res.json()
      if (data.success) setSavedItems(data.data)
    } catch (error) {
  ...
  const fetchRecentActivity = async () => {
    try {
      ...
      const data = await res.json()
      if (data.success) setRecentActivity(data.data.recentActivity?.slice(0, 10) || [])
    } catch (error) {
    ...
    <div className="site-wrap py-8">
      ...
      <div className="mb-8 rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-8">
        <div className="flex items-start justify-between">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-on-surface-variant">
                {profile?.primaryAddress && (
                  <div className="flex items-center gap-1">
                    ...
                    <span>{profile.primaryAddress.city}, {profile.primaryAddress.state}</span>
                  </div>
                )}
                <div className="flex items-center gap-1">
          ...
          {!isEditing ? (
            ...
            <div className="flex gap-2">
              <button onClick={handleSaveProfile} disabled={isSaving} className="btn btn-primary disabled:opacity-50">
                <Save className="h-4 w-4" />
                {isSaving ? 'Saving...' : 'Save'}
              </button>
              <button
                onClick={() => { setIsEditing(false); setEditDisplayName(profile.displayName); setEditInterests(profile.interests) }}
        ...
        <div className="mt-6 rounded-xl bg-surface-container-low p-4 text-sm text-on-surface-variant">
          ...
          <p className="mt-1">
            Your address is used to personalize your feed and is never shared publicly. Only your city and state are visible to others.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/impact" className="flex items-center gap-1 text-sm font-medium text-primary hover:text-primary-container">
          ...
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-6">
            ...
            <div className="space-y-3">
              {recentActivity.map((activity, i) => (
                <div key={i} className="flex items-start gap-3 border-b border-outline-variant/15 pb-3 last:border-0">
                  ...
                  <div className="flex-1">
                    ...
                    <Link href={`/issues/${activity.civicItem.slug}`} className="text-sm text-primary hover:text-primary-container">
                      {activity.civicItem.title}
                    </Link>
                    <p className="mt-1 text-xs text-on-surface-variant">{formatTimeAgo(activity.timestamp)}</p>
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-1">
          <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/saved" className="text-sm font-medium text-primary hover:text-primary-container">View All</Link>
            </div>
            <div className="space-y-3">
              {savedItems.map((item) => (
                <Link
